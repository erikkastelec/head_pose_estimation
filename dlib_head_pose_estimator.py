...
def image_evaluation(path, debug=False, image=None):
    if image is None:
    ...
    if debug:
        ...
        pt2 = [imgpts[0].ravel(), imgpts[1].ravel(), imgpts[2].ravel()]
        sellion_xy = (int(landmarks_2D[7][0]), int(landmarks_2D[7][1]))
        cv2.line(image, sellion_xy, (int(pt2[1][0]), int(pt2[1][1])), (0, 255, 0), 3)  # GREEN
        cv2.line(image, sellion_xy, (int(pt2[2][0]), int(pt2[2][1])), (255, 0, 0), 3)  # BLUE
        cv2.line(image, sellion_xy, (int(pt2[0][0]), int(pt2[0][1])), (0, 0, 255), 3)  # RED
